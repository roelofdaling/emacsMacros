...
(defun imx-pdrain-profiLe-newest ( well )
  ...
  ( let ((ts 0)
	 ...
	 (srchTargetLine "") (shutinWell "")
	 (tabName "") (tabNameTgt "") (day "") (month "") (year "")
	 (pdrain "") (tgttype "") (ltgt "") (ptgt "")
	 (cumBHF "")
	 )
    (setq srchPdrainLine (concat "PDRAIN.*" well))
    (setq srchTargetLine (concat ".*TARGET\\(.*\\)'" well))
    (setq srchInfoLine   "#  Name  Flg Flg  BH Temp")
    (setq srchWellInfo "")
    ...
    (log toBufferName (concat "TABLE (INTEGER ts, LONGTIME TIME, LIQVOLUME cumBHF, PRESSURE pdrain, LIQVOLRATE ltgt, PRESSURE ptgt) " tabName)) 
    (save-excursion
      (beginning-of-buffer)
      (while (re-search-forward srchDateLine nil t)
	(forward-line)
	(setq lastpoint (line-beginning-position))
	(previous-line)      
	(setq ts (+ ts 1) )
	(setq ltgt "0")
	(setq ptgt "0")	
	...
	(previous-line)
	(setq shutinWell (concat "SHUTIN '" well "'"))
        (setq current (buffer-substring (line-beginning-position) (line-end-position) ))
	(if (not (string-match shutinWell current) )
	    (progn
	       (re-search-backward srchTargetLine nil t)
               (setq current (buffer-substring (line-beginning-position) (line-end-position) ))
	       (if (string-match srchTargetLine current)
	          (progn
	            (setq tgttype (match-string 1 current))
	            (setq tgttype (string-trim tgttype))
	            (forward-line)
                    (setq current (buffer-substring (line-beginning-position) (line-end-position) ))
	            (if ( string= tgttype "STO")
		     (setq ltgt (string-trim current))
		      (progn
		       (setq ptgt (string-trim current))
		      )
	            ) 	
	          )  
		 )
	  ...
	  (re-search-backward srchPdrainLine nil t)
          (setq current (buffer-substring (line-beginning-position) (line-end-position) ))
	  ...
	  (re-search-backward srchInfoLine nil t)
	  (setq srchWellInfo (concat "WELL.*'" well "'"))
	  (re-search-forward srchWellInfo nil t)
          (setq current (buffer-substring (line-beginning-position) (line-end-position) ))
	  (setq cumBHF (nth 16 (split-string current)))
	  ;; can compare point with last point to check if we have not gone past it. This would be an error
	  
         (log toBufferName (concat tabName "," (number-to-string ts) ",DATE(" day "," month "," year ")," cumBHF "," pdrain "," ltgt "," ptgt) )
	 (goto-char lastpoint)
        )
     )
  )
...
(defun imx-pdrain-profile-new ( well )
  ...
  ( let ((ts 0)
    ...
    (setq tabName (concat well "_pdrain"))
    (log toBufferName "PRESSURE=KPA")
    (log toBufferName (concat "TABLE (INTEGER ts, LONGTIME TIME, PRESSURE pdrain, LIQVOLRATE ltgt, PRESSURE ptgt) " tabName)) 
    (save-excursion
      (beginning-of-buffer)
	...
	(setq ltgt "0")
	(setq ptgt "0")
	(re-search-forward srchTargetLine nil t)
        (setq current (buffer-substring (line-beginning-position) (line-end-position) ))
	(if (string-match srchTargetLine current)
	    (progn
	      (setq tgttype (match-string 1 current))
	...
	(re-search-forward srchDateLine nil t)
        ...
        (if (string-match srchDate current)
          (progn 
           ...
           (setq day (match-string 2 current))
           (setq year (match-string 3 current))
           (log toBufferName (concat tabName "," (number-to-string ts) ",DATE(" day "," month "," year ")," pdrain "," ltgt "," ptgt) )
          )
        )
      )
    )
